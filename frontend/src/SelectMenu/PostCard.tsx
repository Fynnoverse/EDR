...
import {interlockingTypes, postToInternalIds} from "../config/stations";
...
export const PostCard: React.FC<Props> = ({post, isWebpSupported, controllingPlayer}) => {
    ...
    const postImage = postImages[realId] ?? post.MainImageURL;
    const interlockingType = interlockingTypes[realId];
    ...
    return (
        <Link to={postCardPath} className={`flex flex-col overflow-hidden rounded-lg bg-white shadow-md transition duration-150 ease-out hover:scale-105 active:scale-95 hover:shadow-xl dark:hover:bg-gray-700 dark:border-gray-700 dark:bg-gray-800 hover:bg-gray-100 m-4 w-[250px] cursor-pointer ${realId ? 'opacity-100' : 'opacity-30'}`}>
            <div className="relative h-[110px] w-full bg-gray-300 bg-center bg-cover bg-no-repeat" style={{ backgroundImage: `url(${postImage})`}}>
                {interlockingType &&
                    <Badge
                        className="absolute right-2 top-2 shadow"
                        color={interlockingType === "computer" ? "info" : "warning"}
                    >
                        {t(`SELECTMENU_interlocking_${interlockingType}`, {
                            defaultValue: interlockingType === "computer" ? "Electronic" : "Relay",
                        })}
                    </Badge>
                }
                <div className="absolute bottom-0 flex w-full justify-center bg-white dark:bg-gray-800">
                    {
